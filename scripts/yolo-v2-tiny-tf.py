...
def main(arm_only, asym, per_ch, calibration,
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/yolo-v2-tiny-tf/yolo-v2-tiny-tf.tflite"):
            args['model_path'] = "openvino/public/yolo-v2-tiny-tf/yolo-v2-tiny-tf.tflite"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/yolo-v2-tiny-tf/yolo-v2-tiny-tf.tflite"
    ...
    sdk_net = loaded_net.quantize(
        ...
        model_name="yolo-v2-tiny-tf",
        arm_only=arm_only
    )
    # Execute the quantized net
    sdk_net_output = sdk_net.execute(inputs=inputs)

    saved_model_name = f"yolo-v2-tiny-tf_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"yolo-v2-tiny-tf_asym_{asym}_per_ch_{per_ch}.sima"

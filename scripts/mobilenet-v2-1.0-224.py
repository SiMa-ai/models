...
def main(arm_only, asym, per_ch, calibration,
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/mobilenet-v2-1.0-224/mobilenet_v2_1.0_224.tflite"):
            args['model_path'] = "openvino/public/mobilenet-v2-1.0-224/mobilenet_v2_1.0_224.tflite"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/mobilenet-v2-1.0-224/mobilenet_v2_1.0_224.tflite"
    ...
    sdk_net = loaded_net.quantize(
        ...
        model_name="mobilenet-v2-1.0-224",
        arm_only=arm_only
    )
    # Execute the quantized net
    sdk_net_output = sdk_net.execute(inputs=inputs)

    saved_model_name = f"mobilenet-v2-1.0-224_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"mobilenet-v2-1.0-224_asym_{asym}_per_ch_{per_ch}.sima"

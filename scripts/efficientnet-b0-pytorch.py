...
def main(arm_only, asym, per_ch, calibration,
    ...
    # Model information
    input_names, input_shapes, input_type = ("input", (1, 3, 224, 224), ScalarType.float32)
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/efficientnet-b0-pytorch/efficientnet-b0-pytorch.onnx"):
            args['model_path'] = "openvino/public/efficientnet-b0-pytorch/efficientnet-b0-pytorch.onnx"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/efficientnet-b0-pytorch/efficientnet-b0-pytorch.onnx"
    ...
    if type(input_names) is not list:
        inputs = {InputName(input_names): np.random.rand(1, 3, 224, 224)}
    else:
    ...
    saved_model_name = f"efficientnet-b0-pytorch_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"efficientnet-b0-pytorch_asym_{asym}_per_ch_{per_ch}.sima"

...
def main(arm_only, asym, per_ch, calibration,
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/regnetx-3.2gf/ckpt/regnetx-3.2gf.onnx"):
            args['model_path'] = "openvino/public/regnetx-3.2gf/ckpt/regnetx-3.2gf.onnx"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/regnetx-3.2gf/ckpt/regnetx-3.2gf.onnx"
    ...
    sdk_net = loaded_net.quantize(
        ...
        model_name="regnetx-3.2gf",
        arm_only=arm_only
    )
    # Execute the quantized net
    sdk_net_output = sdk_net.execute(inputs=inputs)

    saved_model_name = f"regnetx-3.2gf_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"regnetx-3.2gf_asym_{asym}_per_ch_{per_ch}.sima"

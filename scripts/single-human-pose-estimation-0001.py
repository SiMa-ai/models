...
def main(arm_only, asym, per_ch, calibration,
    ...
    # Model information
    input_names, input_shapes, input_type = ("input", (1, 3, 384, 288), ScalarType.float32)
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/single-human-pose-estimation-0001/single-human-pose-estimation-0001.onnx"):
            args['model_path'] = "openvino/public/single-human-pose-estimation-0001/single-human-pose-estimation-0001.onnx"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/single-human-pose-estimation-0001/single-human-pose-estimation-0001.onnx"
    ...
    if type(input_names) is not list:
        inputs = {InputName(input_names): np.random.rand(1, 3, 384, 288)}
    else:
    ...
    saved_model_name = f"single-human-pose-estimation-0001_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"single-human-pose-estimation-0001_asym_{asym}_per_ch_{per_ch}.sima"

...
def main(arm_only, asym, per_ch, calibration,
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/resnet-34-pytorch/resnet-34-pytorch.onnx"):
            args['model_path'] = "openvino/public/resnet-34-pytorch/resnet-34-pytorch.onnx"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/resnet-34-pytorch/resnet-34-pytorch.onnx"
    ...
    sdk_net = loaded_net.quantize(
        ...
        model_name="resnet-34-pytorch",
        arm_only=arm_only
    )
    # Execute the quantized net
    sdk_net_output = sdk_net.execute(inputs=inputs)

    saved_model_name = f"resnet-34-pytorch_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"resnet-34-pytorch_asym_{asym}_per_ch_{per_ch}.sima"

...
from afe.load.importers.general_importer import ModelFormat, ImporterParams, tensorflow_source
...
def _get_import_params(*args, **kwargs):
    params = tensorflow_source(*args, **kwargs)
    return params
...
def main(arm_only, asym, per_ch, calibration,
    ...
    # Model information
    input_names, input_shapes, input_type = ("input_1", (1, 416, 416, 3), ScalarType.float32)
    args = {'model_path': '', 'shape_dict': '', 'output_names': ['separable_conv2d_22/BiasAdd']}
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/mobilenet-yolo-v4-syg/mobilenet-yolo-v4-syg.pb"):
            args['model_path'] = "openvino/public/mobilenet-yolo-v4-syg/mobilenet-yolo-v4-syg.pb"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/mobilenet-yolo-v4-syg/mobilenet-yolo-v4-syg.pb"
    ...
    sdk_net = loaded_net.quantize(
        ...
        model_name="mobilenet-yolo-v4-syg",
        arm_only=arm_only
    )
    # Execute the quantized net
    sdk_net_output = sdk_net.execute(inputs=inputs)

    saved_model_name = f"mobilenet-yolo-v4-syg_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"mobilenet-yolo-v4-syg_asym_{asym}_per_ch_{per_ch}.sima"

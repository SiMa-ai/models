...
from afe.load.importers.general_importer import ModelFormat, ImporterParams, onnx_source
...
def main(arm_only, asym, per_ch, calibration,
    ...
    # Model information
    input_names, input_shapes, input_type = ("input", (1, 3, 512, 512), ScalarType.float32)
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/higher-hrnet-w32-human-pose-estimation/ckpt/pose_higher_hrnet_w32_512.onnx"):
            args['model_path'] = "openvino/public/higher-hrnet-w32-human-pose-estimation/ckpt/pose_higher_hrnet_w32_512.onnx"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/higher-hrnet-w32-human-pose-estimation/ckpt/pose_higher_hrnet_w32_512.onnx"
    ...
    if type(input_names) is not list:
        inputs = {InputName(input_names): np.random.rand(1, 3, 512, 512)}
    else:
    ...
    sdk_net = loaded_net.quantize(
        ...
        model_name="higher-hrnet-w32-human-pose-estimation",
        arm_only=arm_only
    )
    # Execute the quantized net
    sdk_net_output = sdk_net.execute(inputs=inputs)

    saved_model_name = f"higher-hrnet-w32-human-pose-estimation_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"higher-hrnet-w32-human-pose-estimation_asym_{asym}_per_ch_{per_ch}.sima"

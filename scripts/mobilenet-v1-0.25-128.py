...
def main(arm_only, asym, per_ch, calibration,
    ...
    # Model information
    input_names, input_shapes, input_type = ("input", (1, 128, 128, 3), ScalarType.float32)
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/mobilenet-v1-0.25-128/mobilenet_v1_0.25_128.tflite"):
            args['model_path'] = "openvino/public/mobilenet-v1-0.25-128/mobilenet_v1_0.25_128.tflite"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/mobilenet-v1-0.25-128/mobilenet_v1_0.25_128.tflite"
    ...
    if type(input_names) is not list:
        inputs = {InputName(input_names): np.random.rand(1, 128, 128, 3)}
    else:
    ...
    sdk_net = loaded_net.quantize(
        ...
        model_name="mobilenet-v1-0.25-128",
        arm_only=arm_only
    )
    # Execute the quantized net
    sdk_net_output = sdk_net.execute(inputs=inputs)

    saved_model_name = f"mobilenet-v1-0.25-128_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"mobilenet-v1-0.25-128_asym_{asym}_per_ch_{per_ch}.sima"

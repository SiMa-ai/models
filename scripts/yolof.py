...
def main(arm_only, asym, per_ch, calibration,
    ...
    # Model information
    input_names, input_shapes, input_type = ("input", (1, 3, 608, 608), ScalarType.float32)
    ...
    if 'model_path' in args.keys():
        if os.path.isfile("openvino/public/yolof/yolof.onnx"):
            args['model_path'] = "openvino/public/yolof/yolof.onnx"
        else:
            args['model_path'] = "/project/jenkinsagents/qa/models/openvino/public/yolof/yolof.onnx"
    ...
    if type(input_names) is not list:
        inputs = {InputName(input_names): np.random.rand(1, 3, 608, 608)}
    else:
    ...
    saved_model_name = f"yolof_asym_{asym}_per_ch_{per_ch}"
    if load_net:
        ...
        load_model_name = f"yolof_asym_{asym}_per_ch_{per_ch}.sima"
